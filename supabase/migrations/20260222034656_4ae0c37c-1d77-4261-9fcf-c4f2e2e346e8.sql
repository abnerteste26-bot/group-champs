...
CREATE POLICY "Admin manage inscricoes"
  ON public.inscricoes_pendentes FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Public insert inscricoes"
  ON public.inscricoes_pendentes FOR INSERT
...
CREATE POLICY "Admin manage campeonatos"
  ON public.campeonatos FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Public read campeonatos"
...
CREATE POLICY "Admin manage grupos"
...
CREATE POLICY "Admin manage grupo_times"
  ON public.grupo_times FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Public read grupo_times"
  ON public.grupo_times FOR SELECT
  USING (true);
...
CREATE POLICY "Admin manage classificacao"
  ON public.classificacao FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Public read classificacao"
  ON public.classificacao FOR SELECT
  USING (true);
...
CREATE POLICY "Admin manage partidas"
  ON public.partidas FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Public read partidas"
  ON public.partidas FOR SELECT
  USING (true);

CREATE POLICY "Time submit result"
  ON public.partidas FOR UPDATE
  TO authenticated
  USING (EXISTS (
    SELECT 1 FROM times t
    WHERE (t.id = partidas.time_a_id OR t.id = partidas.time_b_id)
...
CREATE POLICY "Admin manage times"
  ON public.times FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Public read times"
  ON public.times FOR SELECT
  USING (true);

CREATE POLICY "Time update own"
...
CREATE POLICY "Admin manage timer"
  ON public.campeonato_timer FOR ALL
  TO authenticated
  USING (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Public read timer"
  ON public.campeonato_timer FOR SELECT
  USING (true);
